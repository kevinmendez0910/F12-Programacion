...
class DiaClimatico(RegistroMeteorologico):
    ...
    def descripcion(self):
        ...
        """
        return f"{self.fecha} | max={self.temp_max:.1f}°C  min={self.temp_min:.1f}°C | {self.clasificar()} | Viento: {self.viento_max:.1f} km/h"
...
class RegistroAnual:
    ...
    def dia_mas_caluroso(self):
        ...
        mayor = self._dias[0]
        for dia in self._dias[1:]:
            if dia.temp_max > mayor.temp_max:
                mayor = dia
        return mayor
    ...
    def temp_promedio_anual(self):
        ...
        for dia in self._dias:
            suma += dia.temp_max
        return round(suma / len(self._dias), 1)
    ...
    def resumen(self):
        ...
        """
        tipos = ['Seco', 'Lluvia ligera', 'Lluvia moderada', 'Lluvia intensa']
        caluroso = self.dia_mas_caluroso()

        print(f"Ciudad: {self.ciudad} | Año: {self.anio} | Total días: {len(self)}")
        print(f"Día más caluroso: {caluroso.descripcion()}")
        print(f"Temperatura máxima promedio: {self.temp_promedio_anual()}°C")
        print("\nDías por tipo:")
        for tipo in tipos:
            print(f"  {tipo:<20}: {len(self.dias_por_tipo(tipo))} días")
